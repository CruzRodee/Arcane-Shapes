...
namespace DIALOGUE
{
    public class DL_SPEAKER_DATA
    {
        ...
        public DL_SPEAKER_DATA(string rawSpeaker)
        {
            ...
            // populating default values to avoid null references
            name = "";
            castPosition = Vector2.zero;
            CastExpressions = new List<(int layer, string expression)>();

            // no matches means just a name
            if (matches.Count == 0)
            {
                name = rawSpeaker;
                return;
            ...
            // otherwise we find the speakername from casting data
            ...
            for (int i = 0; i < matches.Count; i++)
            {
                ...
                else if (match.Value == EXPRESSIONCAST_ID)
                {
                    ...
                    endIndex = (i < matches.Count - 1) ? matches[i + 1].Index : rawSpeaker.Length;
                    string castExp = rawSpeaker.Substring(startIndex, endIndex - (startIndex + 1)); // -1 to remove closing bracket
