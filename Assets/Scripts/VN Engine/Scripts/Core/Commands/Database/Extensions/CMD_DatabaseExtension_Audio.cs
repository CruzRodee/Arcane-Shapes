...
namespace COMMANDS
{
    public class CMD_DatabaseExtension_Audio : CMD_DatabaseExtension
    {
        ...
        private static void PlaySFX(string[] data)
        {
            ...
            // Try to get volume
            parameters.TryGetValue(PARAM_VOLUME, out volume, defaultValue: 1.0f);

            // Try to get pitch of sound
            parameters.TryGetValue(PARAM_PITCH, out pitch, defaultValue: 1.0f);

            // Try to get if sound loops
            parameters.TryGetValue(PARAM_LOOP, out loop, defaultValue: false);

            //Run the logic
            AudioClip sound = Resources.Load<AudioClip>(FilePaths.GetPathToResource(FilePaths.resources_audio_sfx, filepath));

            if (sound == null)
            {
                Debug.LogWarning($"[CMD_DatabaseExtension_Audio] PlaySFX: AudioClip not found at path: {filepath}. Please make sure the audio clip is located in a Resources folder.");
                return;
            }
        ...
        private static void PlayVoice(string[] data)
        {
            ...
            //Run the logic
            AudioClip sound = Resources.Load<AudioClip>(FilePaths.GetPathToResource(FilePaths.resources_audio_voices, filepath));
        ...
        private static void PlaySong(string[] data)
        {
            ...
            //Try to get name or path to the track
            parameters.TryGetValue(PARAM_SONG, out filepath);
            filepath = FilePaths.GetPathToResource(FilePaths.resources_audio_music, filepath);

            //Try to get channel for this track
        ...
        private static void PlayAmbience(string[] data)
        {
            ...
            filepath = FilePaths.GetPathToResource(FilePaths.resources_audio_ambience, filepath);
        ...
        private static void PlayTrack(string filepath, int channel, CommandParameters parameters)
        {
            ...
            //Try to get max volume of track
            parameters.TryGetValue(PARAM_VOLUME, out volumeCap, defaultValue: 1.0f);

            //Try to get starting volume of track
            parameters.TryGetValue(PARAM_START_VOLUME, out startVolume, defaultValue: 0.0f);

            //Try to get pitch of track
            parameters.TryGetValue(PARAM_PITCH, out pitch, defaultValue: 1.0f);

            //Try to get if track loops
            ...
            if (sound == null)
            {
                Debug.LogWarning($"[CMD_DatabaseExtension_Audio] PlayTrack: AudioClip not found at path: {filepath}. Please make sure the audio clip is located in a Resources folder.");
                return;
            }
