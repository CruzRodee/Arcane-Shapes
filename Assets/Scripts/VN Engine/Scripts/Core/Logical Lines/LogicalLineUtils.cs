...
namespace DIALOGUE.LogicalLines
{
    public static class LogicalLineUtils
    {
        public static class Encapsulation
        {
            ...
            {
                public bool isNull => lines == null || lines.Count == 0;
        ...
        public static class Expressions
        {
                ...
                for (int i = 0; i < operatorStrings.Count; i++)
                {
                    ...
                    if (operatorString == "*" || operatorString == "/")
                    {
                        ...
                        else if (operatorString == "/")
                        {
                            if (rightOperand == 0)
                            {
                                Debug.LogError("[LogicalLineUtils.Expressions] Division by zero error.");
                                return;
                            }
                            operands[i] = leftOperand / rightOperand;
                        }
            ...
            private static object ExtractValue(string value)
            {
                ...
                if (value.StartsWith(VariableStore.VARIABLE_ID))
                {
                    ...
                    {
                        Debug.LogError($"[LogicalLineUtils.Expressions] Variable '{variableName}' does not exist.");
                        return null;
                    }
        ...
        public static class Conditions
        {
            ...
            public static bool EvaluateCondition(string condition)
            {
                ...
                if (parts.Length == 1)
                {
                    ...
                    {
                        Debug.LogError($"[LogicalLineUtils.Conditions] Unable to evaluate condition: '{condition}'");
                        return false;
                    }
                }
                ...
                {
                    Debug.LogError($"[LogicalLineUtils.Conditions] Unable to evaluate complex condition: '{condition}'");
                    return false;
                }
            ...
            private static bool EvaluateExpression(string left, string op, string right)
            {
                ...
                switch (op)
                {
                    ...
                    case "!=":
                        return left != right;
                    default:
                        Debug.LogError($"[LogicalLineUtils.Conditions] Invalid operator '{op}' for string comparison.");
                        return false;
                }
